# PAPER_PLAN — LMWAM → ICLR 2027(2026-07-24)
...
| **G2 RoboTwin 门** | RoboTwin LMWAM vs baseline 聚合 | ~8月中 | 聚合 SR 显著↑(>2×SEM) | 过→Story A/B 的"headroom 净增益"主结果;不过→转 Story C 诊断论文 |
...
| C3a | 根因一:**目标冗余** — 绝对子目标 86% 是当前态,真信号在残差(判别力 CV 1.1 vs 0.12,10×);flow 条件里 h_t 已在场,绝对目标=重复编码 | pi05 hint 分析 + V8 条件结构论证 | pi05 侧已支持 | **LMWAM 侧闭环 = G1(v8xpb)** | §4 |
...
| C5 | **Headroom 净增益**:LIBERO-10 饱和(94-96)掩盖增益;在 RoboTwin 上 LMWAM 聚合 SR 显著超基线 | robotwin pi05 基线在训(9c8dr);LMWM robotwin 特征已抽 | **needs experiment(G2)** | RoboTwin milestone pairs + LMWM ckpt + LMWAM 训练 + eval | §6 |
...
- 主表 2:RoboTwin 聚合+分任务(baseline vs LMWAM)——**G2 主结果**。
...
- **8/1–8/15**:E4 RoboTwin LMWAM 全线(pairs→LMWM→训练);E7。
...
2. **G1 失败**(残差在 LMWAM 不涨)→ 残差保留为 pi05 侧机理证据;C4 依赖 dual+2Q+tsched,标题去 "Resolving" 改 "Diagnosing…and Mitigating"。
...
**叙事修正**:
1. "LMWAM 追不上 96.4"作废——LaWAM 侧全变体(基线94.2/E1 94.8/2q 94.8/tsched 95.2)**统计平局 @ 94-95 平台**;P1 旧判据(聚合≥96)对着噪声上尾打。
...
## 19. 治理定调(2026-07-28, 用户指令): ICLR LMWAM = 唯一主线
...
## §22 v3.1 初稿落盘(2026-07-28)——三维度收束 + 全文文献接地

**当前标题（2026-08-01 更新）**: *MINT-VLA: Milestone Integration with Native-Space Targets for Vision--Language--Action Models*。MINT-VLA 展开为 *Milestone Integration with Native-Space Targets for Vision--Language--Action Models*；$\pi_{0.5}$ 是当前主要实验实例，不是方法适用范围。

**初稿状态**: `lmvla/paper_iclr_lmvla/main.tex`,12 页,零编译错误,29 条 bib 全部解析。sections/ 下 9 个文件全部完成 v3/v4 重写。

### 三维度主轴(§22 新增——取代旧"现象→根因→修复"线性叙事)

引言前置定纲句:*"we study world-model conditioning along three dimensions --- what to inject, how to integrate it, and how to certify the benefit --- and contribute one result on each"*,四条贡献收束为三条:

| 维度 | 贡献 | 正文 |
|---|---|---|
| **What to inject** | Corpus-scale 95% 冗余测量 → 残差目标 95.75±0.76(no prior art) | §5.1 |
| **How to integrate** | Detach 干预 93.2→95.9(competing gradients 的受控确认) + plateau 反面证据 | §5.2 |
| **How to certify** | 任务级+多 seed+预注册证伪(先杀自己假设) → scope 条件 | §4/§6/§Falsified |

Related Work 同步重组为三段(与三维度镜像),不再按论文家族分。

### 本日完成的主要修改

1. **标题**:论文品牌已统一为 MINT-VLA（Milestone Integration with Native-Space Targets for Vision-Language-Action Models）；标题不绑定 $\pi_{0.5}$，跨架构实证由 T6 单独闭环
2. **摘要 hook**:完全用第三方证据(RoboDojo/WorldArena×2/position paper 原话),不依赖自有实验;"we surveyed"措辞禁止 → "no existing VLA employs, to our knowledge"
3. **引言贡献**:4→3 条,三维度定纲
4. **Related Work**:五段→三段(What/How/Certify),新增辅助损失干扰段落(ForkMerge 诚实校准:不 claim 梯度几何机制)
5. **§4 新增 §4.1**:"What the baseline lacks --- and how it shows"(三级分解 + 第三方佐证),形成完整动机链
6. **全文论断接地**:29 条 bib 每条对应具体论断/number;A 层 15 条已逐条核实,B 层 14 条标 VERIFY
7. **N6 新增**:基线失败模式定性分类(零 GPU)→ 堵 §4.1 缺定性证据的洞

### 剩余缺口(全部有 TODO 锚点)

实验缺口(在飞):P1-F 填充 / combo 臂 / #39 pi05-RoboTwin / P0-B+N2 / E10b 真机
分析缺口(零 GPU,优先):**N6 失败模式分类** / N4 全库分布 / N5 bootstrap CI / N3 附录 / N1 多 seed 补评 / 引文核验 pass
写作缺口:三张表数字回填 + fig2 语料级重画 + 压缩至 9 页

### 关键文件

| 文件 | 作用 |
|---|---|
| `paper_iclr_lmvla/main.tex` + `sections/*.tex` | 论文正文(编译入口 main.tex) |
| `paper_iclr_lmvla/numbers.tex` | 数字宏(所有定量 claims) |
| `paper_iclr_lmvla/refs.bib` | 29 条 bib(每条 note 记已验证数字) |
| `paper_iclr_lmvla/figures/` | 三张图(从 workshop 复制,fig2 待语料级重画) |
| `lmwm/docs/EXPERIMENT_HANDOFF_2026-07-28.md` | P/N 系列实验缺口(含 N6) |
| `memory/project_lmwam_iclr_draft_v3_2026-07-28.md` | 本状态的 memory 入口
