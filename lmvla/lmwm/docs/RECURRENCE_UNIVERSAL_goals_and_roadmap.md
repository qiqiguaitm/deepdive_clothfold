...
- **(a) 已试,负**(2026-07-23):pca256@30Hz = 0.887 < 0.909。**So400m 定格 0.909(pca128@30Hz)**,离线收口。
- **(b) 已开跑(用户拍板)**:AE 臂 A/B —— `ADVANTAGE_TORCH_SO400M`(数据集 so400m_stage,3055ep 零插值)vs 生产 `ADVANTAGE_TORCH_CRAVE_POLY`,唯一变量=标签源(0.909 vs 0.948)。volc `t-20260723173227-wvr7d`(cnsh 8×A100 50k)。判据=AE val 价值损失/Spearman 同水平 → 标签差下游不可见。AWBC 策略臂(pi05_v4_awbc_*)需 dagger 半标签(用冻结 _spec 补),待 AE 臂读数后决定。踩坑:volc SK 含 base64 `=`,awk -F= 截断致 SignatureDoesNotMatch,改 sed 取首 = 后全串。
