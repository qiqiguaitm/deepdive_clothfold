...
python3 "$REPO/lmvla/lmwm/scripts/build_pi05_r4_outcome_manifest.py" \
  --result-root "$BALANCED_A_ROOT" --scene-manifest "$BALANCED_A_SCENES" \
  --behavior-policy "$MODEL/model.safetensors" --output "$BALANCED_A_MANIFEST"
python3 "$REPO/lmvla/lmwm/scripts/build_pi05_r4_outcome_manifest.py" \
  --result-root "$BALANCED_B_ROOT" --scene-manifest "$BALANCED_B_SCENES" \
  --behavior-policy "$MODEL/model.safetensors" --output "$BALANCED_B_MANIFEST"
python3 "$REPO/lmvla/lmwm/scripts/merge_pi05_r4_outcome_manifests.py" \
  --manifest "$BASE_MANIFEST" --manifest "$SUPPORT_MANIFEST" \
  --manifest "$BALANCED_A_MANIFEST" --manifest "$BALANCED_B_MANIFEST" \
