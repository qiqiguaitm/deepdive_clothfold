...
R1_VERIFY_REPO=${R1_VERIFY_REPO:-$REPO}
...
if [[ "$R1_VERIFY_REPO" != "$REPO" || "$TRAIN_SOURCE_REPO" != "$REPO" ]]; then
  test -s "$R1_VERIFY_REPO/REPLICATION_READY"
  test -s "$TRAIN_SOURCE_REPO/REPLICATION_READY"
fi
mkdir -p "$REPO/logs/r1"
python3 "$REPO/lmvla/lmwm/scripts/verify_pi05_r1_protocol.py" \
  --repo "$R1_VERIFY_REPO" --protocol "$PROTOCOL" --output "$REPO/logs/r1/protocol_${ARM}_s${SEED}.json"
...
exec "$REPO/kai0/.venv/bin/python" -u "$TRAIN_SOURCE_REPO/kai0/scripts/train_pi05_robotwin_confirmatory.py" \
  --arm "$TRAIN_ARM" \
  --config-name "$CONFIG" \
  --exp-name "$EXP" \
  --seed "$SEED" \
