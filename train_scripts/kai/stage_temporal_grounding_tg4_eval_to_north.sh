...
PREFETCH_LOCK=$REPO/logs/locks/temporal_grounding_tg4_eval_north_prefetch.lock
...
prefetched=0
uploaded=0
mkdir -p "$(dirname "$PREFETCH_LOCK")"
exec 9>"$PREFETCH_LOCK"
flock 9
while IFS=$'\t' read -r run_id run_name expected size; do
  ...
  if ssh -p "$NORTH_PORT" -o BatchMode=yes "$NORTH_HOST" \
    ...
    reused=$((reused + 1))
  elif ssh -p "$NORTH_PORT" -o BatchMode=yes "$NORTH_HOST" \
      "test -f $(printf %q "$remote_target") && test \"\$(stat -c %s $(printf %q "$remote_target"))\" = $(printf %q "$size") && echo $(printf %q "$expected  $remote_target") | sha256sum -c - >/dev/null"; then
...
ssh -p "$NORTH_PORT" -o BatchMode=yes "$NORTH_HOST" bash -s -- \
  ...
  "$reused" "$prefetched" "$uploaded" <<'REMOTE'
set -Eeuo pipefail
stage=$1
marker=$2
manifest=$3
...
test "$((reused + prefetched + uploaded))" -eq 18
...
printf 'validated=%s\nstage=%s\ncheckpoint_count=18\nreused_north=%s\nreused_prefetch=%s\nuploaded=%s\n' \
  "$(date -u +%FT%TZ)" "$stage" "$reused" "$prefetched" "$uploaded" > "$marker"
REMOTE

mkdir -p "$(dirname "$LOCAL_MARKER")"
printf 'validated=%s\nremote_stage=%s\nremote_marker=%s\ncheckpoint_count=18\nreused_north=%s\nreused_prefetch=%s\nuploaded=%s\n' \
  "$(date -u +%FT%TZ)" "$STAGE" "$REMOTE_MARKER" "$reused" "$prefetched" "$uploaded" > "$LOCAL_MARKER"
