...
INTEGRITY="$REPO/logs/resource_markers/temporal_grounding_tg2r_training_integrity.ok"
...
if [[ "${#checkpoints[@]}" -ne 1 ]]; then
  echo "expected exactly one final TG2R checkpoint for $RUN_ID, found ${#checkpoints[@]}" >&2
  exit 3
fi
CKPT="${checkpoints[0]}"
RUN="$(dirname "$(dirname "$CKPT")")"
RUN_BASENAME="$(basename "$RUN")"
RESULT_NAME="temporal_grounding_tg2r_${ARM}_seed${TRAIN_SEED}"
RESULT_ROOT="$LAWAM/results/eval_runs/robotwin/$RESULT_NAME"
if [[ -e "$RESULT_ROOT" ]]; then
  echo "refusing to mix TG2R eval results with existing path: $RESULT_ROOT" >&2
  exit 3
fi
...
RUN_TAG_PREFIX="tg2r-${ARM}-s${TRAIN_SEED}" \
REPO_ROOT="$REPO" \
