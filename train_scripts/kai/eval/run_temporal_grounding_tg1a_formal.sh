...
FEATURE_ROOT="$REPO/logs/temporal_grounding/tg1a/predicted_endpoint_features"
...
if [[ -e "$RESULT_ROOT" ]]; then
  echo "refusing to mix TG1A results with existing path: $RESULT_ROOT" >&2
  exit 3
fi
if [[ "$CONDITION" == normal && -e "$FEATURE_ROOT" ]]; then
  echo "refusing to overwrite frozen normal feature capture: $FEATURE_ROOT" >&2
  exit 3
fi
...
if [[ "$CONDITION" == shuffled ]]; then
  ...
  test -f "$REPO/logs/temporal_grounding/tg1a/normal_capture_complete.json"
else
...
for seed in 0 1 2 3; do
  ...
  ) >"$LOG_DIR/tg1a_${CONDITION}_seed${seed}_${STAMP}.log" 2>&1 &
  pids+=("$!")
...
if [[ "$status" == 0 && "$CONDITION" == normal ]]; then
  "$REPO/kai0/.venv/bin/python" \
    ...
    --output "$REPO/logs/temporal_grounding/tg1a/normal_capture_complete.json"
fi
echo "TG1A condition=$CONDITION finished status=$status at $(date -u --iso-8601=seconds)"
exit "$status"
