...
"$REPO/kai0/.venv/bin/python" \
  ...
  --repo "$REPO" --manifest "$AMENDMENT" --bundle TG1B
...
if [[ -e "$RESULT_ROOT" ]]; then
  echo "refusing to mix TG1B retry500 results with existing path: $RESULT_ROOT" >&2
  exit 3
fi
...
RUN_TAG_PREFIX="tg1b-retry500-${CHECKPOINT_ARM}-e${CADENCE}" \
REPO_ROOT="$REPO" \
