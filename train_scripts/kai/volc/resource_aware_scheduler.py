...
VISIBLE_NONTERMINAL_JOB_STATES = (*ACTIVE_STATES, *WAITING_STATES)
    ...
    queue["tasks"].append(
        {
            ...
            "priority": 0,
            "description": (
                "Attach four fully utilized East workers to A0 seeds 2/3"
            ),
            "completion_locations": [
                {
                    "label": "accelerator",
...
def add_pi05_r4_outcome_collection_tasks(queue: dict[str, Any]) -> None:
    ...
    managed_ids = {
        ...
        "pi05_r4_eval_north_stage",
        "pi05_r4_terminal_outcome_eval_materialize_north",
        "pi05_r4_outcome_free_crave_eval_materialize_north",
    }
    ...
    north_stage_id = "pi05_r4_eval_north_stage"
    north_stage_marker = (
        REPO / "logs/resource_markers/pi05_r4_eval_north_stage.ok"
    )
    north_stage = Path(NORTH_REPO) / ".staging/pi05_r4_eval_north_v1/repo"
    north_stage_marker_remote = (
        north_stage / "logs/resource_markers/pi05_r4_eval_north_stage.ok"
    )
    queue["tasks"].append(
        {
            "id": north_stage_id,
            "priority": 0,
            "description": (
                "Hash-verify and stage the two pending R4 checkpoints and exact "
                "LeRobot evaluation runtime on North"
            ),
            ...
            "ready_files": [
                ...
                str(REPO / "logs/r4/checkpoint_integrity_v1.json"),
            ...
            "ready_dirs": [
                "/vePFS/tim/workspace/lerobot-pi05-server-venv",
                "/vePFS/tim/.uv_python/cpython-3.12.13-linux-x86_64-gnu",
                "/vePFS/tim/workspace/lerobot-main/src",
    ...
    north_abi_repair = (
        REPO
        / "lmvla/paper_iclr_lmvla/manifests/"
        "pi05_r4_north_python_abi_repair_v1.json"
    )
    north_abi_spec = json.loads(north_abi_repair.read_text())
    north_abi_ready_hashes = [
        {"path": str(REPO / relative), "sha256": expected}
        for relative, expected in sorted(north_abi_spec["file_sha256"].items())
    ]
    north_abi_marker = (
        REPO / "logs/resource_markers/pi05_r4_north_python_abi_repair.ok"
    )
    north_abi_marker_remote = (
        north_stage / "logs/resource_markers/pi05_r4_north_python_abi_repair.ok"
    )
    queue["tasks"].append(
        {
            "id": "pi05_r4_north_python_abi_repair",
            "priority": 0,
            "description": (
                "Repair and preflight Python 3.10/3.12 isolation in the staged "
                "North RoboTwin evaluator"
            ),
            "completion_glob": str(north_abi_marker),
            "completion_min_count": 1,
            "rearm_after_ready_file": str(north_abi_repair),
            "ready_files": [
            ...
            "ready_hashes": [
                *north_abi_ready_hashes,
                {
                    "path": str(north_abi_repair),
                    "sha256": sha256_file(north_abi_repair),
                },
            ],
            "candidates": [
                {
                    ...
                    "max_failures": 3,
                    "status_dir": str(REPO / "logs/r4/north_python_abi_repair/launcher"),
                    "command": shlex.join(
                        [
                            "bash",
                            str(
                                REPO
                                / "train_scripts/kai/"
                                "repair_pi05_r4_north_python_abi.sh"
                            ),
                        ]
                    ),
                }
    ...
    for arm in ("terminal_outcome", "outcome_free_crave"):
        eval_task_id = f"pi05_r4_{arm}_seed1000_eval"
        parent = next(task for task in queue["tasks"] if task.get("id") == eval_task_id)
        result_name = f"pi05_r4_{arm}_seed1000"
        local_marker = REPO / "logs/resource_markers" / f"{result_name}.ok"
        remote_marker = north_stage / "logs/resource_markers" / f"{result_name}.ok"
        remote_model = (
            north_stage
            / "lmvla/lmwm/checkpoints/pi05_r4_matched_v1"
            / f"{arm}-seed1000/checkpoints/005000/pretrained_model"
        ...
        parent_hashes = {
            item["path"]: item["sha256"] for item in parent["ready_hashes"]
        }
        parent_hashes.update(
            {item["path"]: item["sha256"] for item in north_eval_ready_hashes}
        )
        parent_hashes.update(
        ...
        materialize_id = f"pi05_r4_{arm}_eval_materialize_north"
        queue["tasks"].append(
            {
                "id": materialize_id,
                "priority": 0,
                "description": f"Verify and materialize North R4 {arm} evaluation",
                "materialize_north_result_for": eval_task_id,
                "completion_glob": str(local_marker),
                "completion_min_count": 1,
                "ready_files": [
                    str(north_eval_amendment),
                ...
                "candidates": [
                    {
                        ...
                        "retry_cooldown_seconds": 60,
                        "status_dir": str(
                            REPO / "logs/r4/north_eval_materialize" / arm
                        ),
                        "command": shlex.join(
                            [
                                "env",
                                ...
                                str(
                                    REPO
                                    / "train_scripts/kai/"
                                    "sync_pi05_r4_eval_from_north.sh"
                                ),
                            ]
                        ),
                    }
...
def add_pi05_r4_replication_tasks(queue: dict[str, Any]) -> None:
    ...
    north_spec = json.loads(north_amendment.read_text())
    north_ready_hashes = [
        {"path": str(REPO / relative), "sha256": expected}
        for relative, expected in sorted(north_spec["file_sha256"].items())
    ]
    north_stage_id = "pi05_r4_replication_north_stage"
    north_stage_marker = (
        REPO / "logs/resource_markers/pi05_r4_replication_north_stage.ok"
    )
    north_repo = Path(
        "/vePFS-North-E/vis_robot/tim/workspace/deepdive_kai0"
    )
    north_stage_marker_remote = (
        ...
        queue["tasks"].append(
            {
                ...
                "priority": 1,
                "description": (
                    "Byte-verify the frozen R4 replication data and runtime on North"
                ),
                "completion_glob": str(north_stage_marker),
                "completion_min_count": 1,
                ...
                "candidates": [
                    {
                        ...
                        "retry_cooldown_seconds": 300,
                        "max_failures": 3,
                        "status_dir": str(REPO / "logs/r4/north_replication_stage"),
                        "command": shlex.join(["bash", str(stage_script)]),
                    }
                ],
            }
        )
    ...
    for seed in (1001, 1002):
        for arm in ("ordinary", "terminal_outcome", "outcome_free_crave"):
            ...
            if train_id not in existing:
                remote_marker = (
                    north_repo / "logs/resource_markers" / f"pi05_r4_{run_name}.ok"
                )
                queue["tasks"].append(
                    {
                        "id": train_id,
                        ...
                        "completion_locations": [
                            {
                                "label": "shared",
                                "glob": str(train_marker),
                                "remote": False,
                            },
                            {
                                "label": "north",
                                "glob": str(remote_marker),
                                "remote": True,
                            },
                        ],
                        ...
                        "candidates": [
                            ...
                            },
                            {
                                ...
                                "yaml": (
                                    "train_scripts/kai/volc/"
                                    "pi05_r4_replication_train_north_4h20.yaml"
                                ),
                                "task_name": (
                                    f"pi05-r4-{arm.replace('_', '-')}-s{seed}-north4g"
                                ),
                                "env": {"R4_ARM": arm, "R4_SEED": str(seed)},
                                "ready_files": [
                                    str(north_stage_marker),
                                ...
                                "ready_files_remote": [
                                    ...
                                    *(
                                    ...
                                    str(
                                        north_repo
                                        / "lmvla/lmwm/data/pi05_r4_training_v1/"
                                        "lerobot_query_chunks/meta/info.json"
                                    ),
                                    (
                                        "/vePFS-North-E/vis_robot/tim/hf_models/"
                                        "SidneyXie_pi05_robotwin/model.safetensors"
                                    ),
                                ],
                            }
                        ],
...
def add_temporal_grounding_tasks(queue: dict[str, Any]) -> None:
    ...
    tg2_recovery_post_v2_path = (
        manifests / "temporal_grounding_tg2_recovery_posttraining_v2.json"
    )
    tg2_recovery_post_v3_path = (
        manifests / "temporal_grounding_tg2_recovery_posttraining_v3.json"
    )
    ...
    tg2_recovery_post_v3_replaced_files = set(tg2_recovery_post_v3["files"])
    tg2_recovery_post_v3_hashes = [
        ...
        {
            "path": str(tg2_recovery_post_v2_path),
            "sha256": manifest_hashes[tg2_recovery_post_v2_path],
        },
        {
            "path": str(tg2_recovery_post_v3_path),
            "sha256": manifest_hashes[tg2_recovery_post_v3_path],
        },
        *(
            {"path": str(REPO / relative), "sha256": digest}
            for relative, digest in tg2_recovery_post["files"].items()
            if relative not in tg2_recovery_post_v3_replaced_files
        ),
        *(
            {"path": str(REPO / relative), "sha256": digest}
            for relative, digest in tg2_recovery_post_v2["files"].items()
            if relative not in tg2_recovery_post_v3_replaced_files
        ),
        *(
            {"path": str(REPO / relative), "sha256": digest}
    ...
    tg2_recovery_post_v6_replaced_files = set(tg2_recovery_post_v6["files"])
    tg2_recovery_post_v6_hashes = [
        *(
            item
            for item in tg2_recovery_post_v5_hashes
            if str(Path(item["path"]).relative_to(REPO))
            not in tg2_recovery_post_v6_replaced_files
        ),
        {
            "path": str(tg2_recovery_post_v6_path),
            "sha256": manifest_hashes[tg2_recovery_post_v6_path],
        },
        *(
            {"path": str(REPO / relative), "sha256": digest}
                ...
                *(
                    {
                        ...
                        "glob": str(
                            tg1a_shuffled_root
                            / f"seed{seed}/**/tasks/stack_blocks_three/run.log"
                        ),
                        "regex": r"progress:.*?([0-9]+)/([0-9]+)",
                    }
                    for seed in range(4)
    ...
    east_overlay = Path("/vePFS/tim/runtime/tg2_transformers_5_2_py312_padding_v3")
    east_overlay_hashes = {
        ...
        east_overlay / "sitecustomize.py": "922e7b08959d01faad7ae64b04114aebdf6cd71666af6d49dfafe93380de4f90",
    }
    tg2_hashes = [
    ...
    tg4_north_terminal_recovery_runner = (
        REPO / "train_scripts/kai/recover_temporal_grounding_tg4_north_terminal.sh"
    )
    tg4_north_terminal_recovery_cells = {
        ("clean_base", 1100),
        ("clean_base", 1101),
        ("clean_base", 1102),
        ("auxiliary_only", 1102),
    ...
    for recovery_arm, recovery_seed in sorted(tg4_north_terminal_recovery_cells):
        ...
        recovery_id = f"{recovery_run_id}_north_terminal_recovery"
        recovery_marker = (
            REPO / "logs/resource_markers" / f"{recovery_id}.json"
        )
        tg4_north_terminal_markers[(recovery_arm, recovery_seed)] = recovery_marker
        recovery_task = {
            "id": recovery_id,
            "priority": 0,
            "description": (
                "Audit a TG4 North cell launched through the mutable legacy runner "
            ...
            "ready_hashes": [
                ...
                },
                {
                    ...
                    "sha256": "0b13f31f18933383da1cc1f402ccd766799d2ac15940dadab45cbc6a689290d8",
                },
            ],
            "candidates": [
                {
                    ...
                    "status_dir": str(
                        REPO
                        / "logs/temporal_grounding/tg4/north_terminal_recovery"
                        / recovery_run_id
                    ),
                    "command": shlex.join(
                        [
        ...
        for seed in (1100, 1101, 1102):
            ...
            gf1_runtime_revision = (
                ...
                if conditioning_ddp_repair
                    ...
                    "temporal_grounding_tg4_gf1_v6_queue_recovery"
                    if gf1_queue_recovery
                    else "temporal_grounding_tg4_gf1_v3"
                )
            )
            gf1_status_suffix = "_queue_recovery_v3" if gf1_queue_recovery else ""
            task = {
                "id": task_id,
                "priority": 0,
    ...
    tg4_eval_gf1_preflight_marker = (
        REPO
        / "logs/resource_markers/temporal_grounding_tg4_eval_gf1_preflight.ok"
    )
    ...
    tg4_eval_gf1_preflight_task = {
        "id": tg4_eval_gf1_preflight_id,
        "priority": -1,
        "description": (
            "Non-claim-bearing one-episode gf1 preflight of the frozen TG4 "
            "full-arm evaluator"
        ),
        "allow_temporary_gf1": True,
        ...
        "ready_files": [
            ...
            str(tg4_renderer_env),
            str(tg4_renderer_wrapper),
            str(tg4_north_terminal_markers[("full", 1100)]),
        ],
        "ready_globs": [
            str(
                REPO
    ...
    if tg4_eval_gf1_preflight_id not in existing:
        queue["tasks"].append(tg4_eval_gf1_preflight_task)
        existing.add(tg4_eval_gf1_preflight_id)
    else:
    ...
    order_probe_yaml = (
        REPO
        / "train_scripts/kai/volc/temporal_grounding_tg2_data_order_probe_runtime_v2_east_4h20.yaml"
    )
    order_probe_runner = (
    ...
    for arm in ("future_off", "fixed_endpoint", "raw_milestone"):
        for seed in (1000, 1001, 1002):
            ...
            if task_id not in existing:
                ...
                next(
                    task for task in queue["tasks"] if task.get("id") == task_id
                ).update(materialize_task)
        ...
        "candidates": [
            {
                ...
                "yaml": str(recovery_integrity_yaml.relative_to(REPO)),
                "task_name": "temporal-grounding-tg2r-integrity-east1g",
            }
        ],
    }
    if recovery_integrity_id not in existing:
        queue["tasks"].append(recovery_integrity_task)
        existing.add(recovery_integrity_id)
    else:
        next(
            task
            for task in queue["tasks"]
            if task.get("id") == recovery_integrity_id
        ).update(recovery_integrity_task)
    ...
    for arm in ("future_off", "fixed_endpoint", "raw_milestone"):
        for seed in (1000, 1001, 1002):
            ...
            if task_id in existing:
                ...
                persisted["ready_files"] = [
                    *persisted.get("ready_files", []),
                    str(tg2_recovery_post_v2_path),
                    str(tg2_recovery_post_v3_path),
                    str(tg2_recovery_post_v4_path),
                    str(tg2_recovery_post_v5_path),
                    ...
                    *(
                        str(REPO / relative)
                        for relative in tg2_recovery_post_v3["files"]
                    ),
                    *(
                        str(REPO / relative)
                        for relative in tg2_recovery_post_v4["files"]
                    ),
                ]
                persisted["ready_files"] = list(
...
def list_jobs(
    ...
    for state in VISIBLE_NONTERMINAL_JOB_STATES:
...
def write_markdown_snapshot(snapshot: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Beijing GPU Quotas",
            "",
            "| Credential | Active GPUs | Queued GPUs | GPU limit |",
            "|---|---:|---:|---:|",
            (
                "| primary | "
