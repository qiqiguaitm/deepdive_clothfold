...
| ⭐⭐ P1 | [**CRAVE value 打标 × 新 dagger(chunk-001) 验证**](plans/crave_value_dagger_validation_plan.md) | ✅ Phase A 打标完成. ✅ Phase B: build 774ep + discretize top30% + config `pi05_v4_awbc_chunk001_dagger_crave` + **已提交 16 卡 `t-20260715142954-t6dgl`** (cnsh robot-task, 2×8 A100, 50k) | 16× A100 cnsh robot-task | — | CRAVE pipeline 给新 chunk-001 dagger 全量打标完成→build数据集+discretize→AWBC 16卡训练中→真机:新格式是否天然不冻 |
...
| ~~CRAVE polyline 标签 × 重训 KAI0 pi0-AE~~ | [(plan)](plans/crave_polyline_kai_ae_retrain_plan.md) | ❌ **此路不通** (2026-07-15): v1(value-only) + v2 MT(loss_action=1.0) 两版均 dead-value (Spearman 0.10, value 塌~0.02). "加 action 辅助任务救 value" 假设证伪. pi0-AE value_head 学不出 CRAVE 监督 → 改用在线 GRU (corr 0.975) | — | — | 结论: pi0 backbone 的 torch value_head 路线放弃; CRAVE 进度信号走独立 GRU (见 crave_value_dagger_validation) |
