...
_LEGACY_DATA_DIR = _PROJECT_ROOT / "train_scripts" / "kai" / "data"
...
def _maybe_norm_stats(dst, compute: bool, action_dim: int):
    ...
    manual = f"python {_LEGACY_DATA_DIR}/norm_stats_from_dataset.py {dst} --action-dim {action_dim}"
    if not compute:
        print(f"  [norm_stats] skipped (--no-norm-stats). Run manually: {manual}", flush=True)
        return
    try:
        sys.path.insert(0, str(_LEGACY_DATA_DIR))
